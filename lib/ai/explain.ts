...
function systemPrompt(level: Level): string {
  ...
  return [
    ...
    `- subject: وصف إنجليزي يحدد الهوية البصرية الأساسية. استخدم دائماً: "${visualStyle}".`,
    `- visual (لكل خطوة): وصف إنجليزي مركز يصور المكونات في حالة حركة أو تفاعل. ادمج "${visualStyle}".`,
    `- imagePrompt: وصف هندسي وفني متكامل يجمع المشهد الكلي أو مقطعاً عرضياً تفصيلياً. استخدم مفردات مثل "micro-detail, macro lens, hyper-realistic, 8k".`,
    `- ${noTextConstraint}`,
    '',
