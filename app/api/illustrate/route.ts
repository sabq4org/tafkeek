...
const bodySchema = z.object({
  ...
  steps: z
    .array(z.object({ order: z.number().int(), visual: z.string().min(4) }))
    .min(1)
    .max(8),
});
...
export async function POST(req: NextRequest) {
  ...
  try {
    cover = await generateIllustration(
      `${subject}. A clear full view of the whole subject.`,
    );
  } catch (err) {
  ...
  const images = await mapWithConcurrency(steps, 3, async (step) => {
    try {
      const image = await generateIllustration(step.visual, cover ?? undefined);
